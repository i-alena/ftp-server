...
bool fileoperator::changeDir(std::string newPath, bool strict) {
	...
	if ((newPath.compare("..") == 0) || (newPath.compare("../") == 0)) {
		...
		if (this->completePath.size() <= 1) {
			...
			std::cout << "this->completePath.pop_back();\n";
	...
	std::cout << "Try to open: " << this->getCurrentWorkingDir() + newPath << std::endl;
	if (this->dirCanBeOpenend(this->getCurrentWorkingDir().append(newPath))) {
		...
		std::cout << "completePath.push_back(newPath)\n";
		return (EXIT_SUCCESS); // 0
	}
	else {

		std::cout << "dir Can not Be Openend " << std::endl;
		return (EXIT_FAILURE); // 1
	}
}
...
bool fileoperator::createDirectory(std::string& dirName, bool strict) {
	...
	return ((_mkdir(this->getCurrentWorkingDir().append(dirName).c_str()) == 0) ? EXIT_SUCCESS : EXIT_FAILURE);
}
...
//Deletes a directory on the server given by its name recursive, since a directory can only be deleted if it is empty
bool fileoperator::deleteDirectory(std::string dirName, bool cancel, std::string pathToDir) {
	...
	// Walk over all files in the current directory and delete them
	...
	if ((pathToDir.compare(".") != 0) && (pathToDir.compare("..") != 0)) {
		if (_rmdir(this->getCurrentWorkingDir().append(pathToDir).c_str()) < 0) {
			std::cerr << "Failed deleting directory '" << this->getCurrentWorkingDir(false).append(pathToDir) << "'" << std::endl; // 0 == success, -1 == error
		}
		else {
	...
	for (singleDir = this->completePath.begin(); singleDir != this->completePath.end(); ++singleDir) {
		...
		if (singleDir == this->completePath.begin()) {
			...
			if (!showRootPath) {
			 ...
			 // The lowest path already is chdir()'d into
...
void fileoperator::browse(std::string dir, std::vector<std::string>& directories, std::vector<std::string>& files, bool strict) {
	...
	if (dir.compare("./") != 0) {
		...
		std::cout << "dir.compare(. /) != 0" << std::endl;
	}
	else {
